...
export default function AdminPage() {
  ...
  const handleUpdateStatus = async (requestId: number, status: 'approved' | 'rejected') => {
    try {
      const response = await apiService.updateRequestStatus(requestId.toString(), status);
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  <div className="flex-shrink-0">
                    <div className="relative w-16 h-20">
                      <Image
                        src={request.cover_image || '/images/placeholder-book.jpg'}
                  ...
                  <div className="flex-grow">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 text-sm mb-1">by {request.book_author}</p>
                        <p className="text-gray-500 text-xs mb-2">ISBN: {request.isbn}</p>
                        <p className="text-gray-500 text-xs">{request.publisher}</p>
                      </div>

                      {/* User Information */}
                      <div>
                        <h4 className="font-semibold text-gray-800 mb-2">Requestor Details</h4>
                        <div className="space-y-1 text-sm">
                          <p><span className="font-medium">Name:</span> {request.user_name}</p>
                          <p><span className="font-medium">Username:</span> {request.username}</p>
                          <p><span className="font-medium">Email:</span> {request.email}</p>
                          <p><span className="font-medium">Role:</span> {request.academic_role}</p>
                          <p><span className="font-medium">ID:</span> {request.no_induk}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">{new Date(request.tanggal_pinjam).toLocaleDateString()}</p>
                      </div>
                      <div>
                        <span className="font-medium text-gray-700">Due Date:</span>
                        <p className="text-gray-600">{new Date(request.tenggat_pengembalian).toLocaleDateString()}</p>
                      </div>
                    </div>
