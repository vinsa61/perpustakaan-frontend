...
export default function BookCard({ book, onBorrowSuccess }: BookCardProps) {
  ...
  const formatAuthors = (pengarang: Buku['pengarang']) => {
    ...
    if (Array.isArray(pengarang)) {
      return pengarang.map(author => `${author.nama_depan} ${author.nama_belakang}`).join(', ')
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <div className="relative h-64 w-full">
        <Image
          src={book.cover_image || '/images/placeholder-book.jpg'} // Fixed: removed array access
          alt={book.judul}
          fill
      ...
      <div className="p-4">
        <h3 className="font-semibold text-lg mb-1 line-clamp-2">{book.judul}</h3>
        ...
        <div className="flex justify-between items-center">
          ...
          {user ? (
            <button
              ...
              style={{
                backgroundColor: isAvailable && !borrowing ? '#879D82' : undefined
              }}
            >
              {borrowing ? (
